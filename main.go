...
func ensureCodexCLIReady(binary string) (string, string, error) {
	...
	if err != nil {
		return "", "", errors.New(codexMissingInstallHint(runtime.GOOS, name))
	}
	...
	if err != nil {
		return "", "", errors.New(codexExecutableInstallHint(runtime.GOOS, path))
	}
...
func codexMissingInstallHint(goos string, binary string) string {
	name := strings.TrimSpace(binary)
	if name == "" {
		name = "codex"
	}
	if goos == "windows" {
		return fmt.Sprintf("Codex CLI tidak ditemukan. Install Codex secara manual lalu pastikan `%s`, `codex.cmd`, atau `codex.exe` tersedia di PATH. Jika perlu, set `CODEXSESS_CODEX_BIN` ke path penuh binary Codex.", name)
	}
	return fmt.Sprintf("codex CLI not found in PATH. install manually with: npm i -g @openai/codex, or set CODEXSESS_CODEX_BIN to the full binary path for %s", name)
}

func codexExecutableInstallHint(goos string, path string) string {
	resolved := strings.TrimSpace(path)
	if resolved == "" {
		resolved = "codex"
	}
	if goos == "windows" {
		return fmt.Sprintf("Codex CLI ditemukan tetapi gagal dijalankan (%s). Verifikasi instalasi manual Codex di Windows lalu pastikan PATH atau `CODEXSESS_CODEX_BIN` mengarah ke binary yang benar.", resolved)
	}
	return fmt.Sprintf("codex CLI is not executable (%s). verify the manual installation and PATH, or set CODEXSESS_CODEX_BIN to a working binary", resolved)
}
...
func waitForExitOnWindowsError() {
	...
	if !term.IsTerminal(int(os.Stdin.Fd())) {
		_ = exec.Command(
			...
			"-NoProfile",
			"-Command",
			`Write-Host "Press Enter to close..." -NoNewline; [void][System.Console]::ReadLine()`,
		).Run()
		return
	}
	fmt.Print(windowsExitPrompt())
